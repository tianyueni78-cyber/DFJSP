...
| [阶段 A 完全重调度确认运行](docs/stage_a_confirmation_search.md) | 10 个体、20 代确认运行的参数、输出和 MATLAB 入口 |
...
- 完全重调度 10×20 确认运行已执行两次，固定种子结果一致，运行时间约 3.28–4.03 秒；
- 已完成 Pareto 目标去重及“连续无改善/时间上限/最大代数”自适应终止代码，MATLAB 回归测试待执行；
- 尚未运行完全重调度搜索，也尚未计算 `tD`、`SD` 和 `Y`。

## 下一步执行边界

下一步先运行确认配置测试，再由用户启动 10×20 完全重调度确认运行。该运行不是 50×100 正式实验。
