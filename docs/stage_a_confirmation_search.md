...
种群、交叉和变异参数参考原项目入口。前一次最大 20 代运行在第 19 代仍发生改善，并最终以 `maximum_generations` 停止，因此本次只把最大代数提高到 100，让无改善条件或时间上限决定是否提前结束。新搜索器不会采用原 `INSGA_II.m` 内部强制覆盖为 150 的行为。
