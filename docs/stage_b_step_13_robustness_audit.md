...
| 种子 | 停止原因 | 代数 | 用时/秒 | Pareto 数 | 最终策略 | 最终卸载 | tD | SD | Y |
|---:|---|---:|---:|---:|---|---:|---:|---:|---:|
| 11 | 达到最大代数 | 100 | 22.0300 | 3 | 完全重调度 | 93.333 | -50.870 | 24 | -43.383 |
| 22 | 连续无改善 | 32 | 7.7326 | 1 | 完全重调度 | 117.330 | -26.870 | 31 | -21.083 |
| 33 | 连续无改善 | 30 | 6.8363 | 3 | 完全重调度 | 121.710 | -22.493 | 36 | -16.644 |
| 42 | 连续无改善 | 47 | 10.3760 | 1 | 完全重调度 | 96.163 | -48.040 | 36 | -39.636 |
| 55 | 连续无改善 | 45 | 9.9458 | 3 | 完全重调度 | 111.830 | -32.370 | 37 | -25.433 |

五次运行全部选择完全重调度，种子 `42` 复现第 12 步正式结果。

## 稳健性结论

- 当前 `omega1=0.9、omega2=0.1` 权重下，最终策略对五个随机种子稳定；
- 五次运行全部选择完全重调度，选择比例为 `5/5`；
- 最终卸载时间范围约为 `93.333` 至 `121.710`；
- `Y` 范围约为 `-43.383` 至 `-16.644`；
- 搜索到的解质量随随机种子变化，因此不能把单次最优数值视为确定结果；
- 但所有种子的完全重调度方案均优于局部右移的 `Y=0`；
- 结合权重分析，当前结论应表述为：当完工时间权重不低于 `0.5` 时，
  完全重调度具有稳定选择优势；当更重视机器分配稳定性时，局部右移更合适。
...
每次重新运行会保存新的 `result.mat` 和 `multiseed_summary.csv`，不覆盖
已有结果。
