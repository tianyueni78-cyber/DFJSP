...
基于 `outputs/stage_cs2_complete_reschedule_search/20260616_150249/result.mat`，
组合选择结果为：

- 选中策略：`complete_rescheduling`；
- 正常基线最终卸载时间：`144.2033`；
- 完全重调度最终卸载时间：`122.0900`；
- 完工时间偏差 `tD=-22.1133`；
- 机器分配偏差 `SD=23`；
- 组合指标 `Y=-17.6020`；
- 局部右移方案：`tD=0`、`SD=0`、`Y=0`；
- 可重调工序数：`40`；
- `all_constraint_audits_validated=1`；
- `all_energy_audits_complete=1`。

解释：在当前权重 `omega1=0.9, omega2=0.1` 下，完全重调度虽然改变了
`23` 个可重调工序的机器分配，但最终卸载时间减少约 `22.1133`，综合指标
低于局部右移，因此被选为 C-S2 当前正式方案。
