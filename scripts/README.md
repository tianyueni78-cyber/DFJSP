...
`run_stage_c_simultaneous_reschedule_operators.m`

使用固定随机种子建立受限父代并执行一次交叉和变异。只改变未开工任务的
OS、MS、AS、SS，不评价适应度、不选择幸存者、不运行迭代搜索。

## 阶段 C 第 10.3 步

`run_stage_c_simultaneous_restricted_search_contract.m`
...
`run_stage_c_plan_version_history.m`

把正常基线记录为 `V0`，把第 11 步选定方案记录为 `V1`，并验证事件时刻
查询当前生效版本。本入口不处理下一故障、不运行搜索、不生成输出。

## 阶段 C 第 13 步

`run_stage_c_next_fault_state.m`

从 `V1` 计划动态筛选下一有效故障并提取工序与 AGV 状态，记录仍活动的旧
维修区间。本入口不传播影响、不修改计划、不运行搜索。

## 阶段 C 第 14 步

`run_stage_c_sequential_impact_context.m`

累计历史与新故障维修区间，传播新故障影响并合并仍有效的历史影响。本入口
不修改机器或 AGV 时间表、不追加版本、不运行搜索。

## 阶段 C 第 15 步

`run_stage_c_sequential_agv_linked_right_shift.m`

从第 14 步当前计划和连续故障影响集合生成局部右移机器候选，识别并调整
AGV 运输约束。本入口不追加新计划版本、不运行完全重调度搜索。

## 阶段 C 第 16.1 步

`run_stage_c_sequential_frozen_problem.m`

建立连续故障完全重调度冻结问题，冻结已完成和在制任务，释放未开工任务。
本入口不解码候选、不运行搜索、不执行组合选择。

## 阶段 C 第 16.2 步

`run_stage_c_sequential_complete_reschedule_decode.m`
...
`run_stage_c_sequential_complete_search.m`

运行连续故障单随机种子正式搜索，并保存 MAT、Pareto CSV、历史 CSV 和运行
摘要。该入口会生成输出，运行前必须单独确认。

## 阶段 C 第 16.5 步

`run_stage_c_sequential_combination_selection.m`

复用一个连续故障搜索结果，重建同一当前计划和下一故障下的局部右移方案，
计算 `tD、SD、Y` 并执行约束、最终卸载和能耗审计。

## 阶段 C 第 16.6 步

第 16.6 步不新增脚本，只整理第 16.4 的正式搜索输出和第 16.5 的组合评价结果。

## 阶段 C 第 17.1 步

第 17.1 步不新增脚本，只建立最终审计场景矩阵配置。

## 阶段 C 第 17.2 步

`run_stage_c_final_audit_multiseed.m`

按最终审计矩阵筛出已经实现的 `C-S1` 与 `C-SEQ1`，分别运行
`[11,22,33,42,55]` 五个随机种子的正式搜索，随后执行组合选择并保存
多种子汇总。该入口会生成输出，运行前必须单独确认。

## 阶段 C 第 18 步

第 18 步不新增脚本，只整理阶段 C 最终总结和代码导读。

## C-S2 第 1 步

`run_stage_cs2_restart_commitments.m`

复用 Stage C 同时故障场景，把中断规则改为 `restart_from_zero`，并为多个
直接中断工序建立从头加工承诺。本入口不传播影响、不修改机器或 AGV 时间表、
不运行搜索。

## C-S2 第 2 步

`run_stage_cs2_impact_analysis.m`

从 C-S2 第 1 步的从头加工承诺出发，使用完整重加工完成时间传播工件后继
和机器后继影响。本入口不修改机器或 AGV 时间表、不运行搜索。

## C-S2 第 3 步

`run_stage_cs2_machine_right_shift.m`

将 C-S2 损失加工段、完整重加工段和受影响后续工序投影时间写入机器候选
表。本入口不调整 AGV、不运行搜索。

## C-S2 第 4 步

`run_stage_cs2_agv_impact_analysis.m`

在 C-S2 机器局部右移候选上识别失效运输和同 AGV 后续复核运输。本入口
不修改 AGV 时间表、不运行搜索。

## C-S2 第 5 步

`run_stage_cs2_agv_linked_right_shift.m`

正式调整 C-S2 受影响 AGV 运输任务，并把运输延迟反馈到机器工序开工时间，
生成机器 + AGV 联动局部右移候选。本入口不运行搜索。

## C-S2 第 6 步

`run_stage_cs2_frozen_problem.m`

建立 C-S2 从头加工规则下的完全重调度冻结问题，固定已完成、在制、中断
承诺、维修区间和资源边界。本入口不运行搜索。

## C-S2 第 7 步

`run_stage_cs2_complete_reschedule_decode.m`

基于 C-S2 冻结问题解码一个基线种子决策，生成完全重调度候选。本入口
不运行搜索。

## C-S2 第 8 步

`run_stage_cs2_reschedule_operators.m`

基于 C-S2 冻结问题生成受限种群，并执行一次交叉/变异。所有个体使用
C-S2 第 7 步解码器验证。本入口不评价适应度、不运行搜索。

## C-S2 第 9 步

`run_stage_cs2_restricted_search_contract.m`

使用 6 个个体、2 代的轻量配置运行 C-S2 受限 NSGA-II 搜索契约，验证候选
评价、Pareto 去重和自适应停止分支。本入口不生成正式实验输出。

## C-S2 第 10 步

`run_stage_cs2_complete_search.m`

运行 C-S2 同时故障从头加工规则的单随机种子正式完全重调度搜索，并保存
MAT、Pareto CSV、历史 CSV 和运行摘要。该入口会生成输出，运行前必须单独确认。

## C-S2 第 11 步

`run_stage_cs2_combination_selection.m`

复用 C-S2 正式搜索结果，重建同一同时故障从头加工场景下的局部右移方案，
计算 `tD、SD、Y`，并执行 C-S2 专用从头加工、维修区间、最终卸载和能耗审计。
...
`run_stage_cseq2_unavailability_context.m`

在 C-SEQ2 第 1 步的重叠连续故障状态上，保留历史维修并加入新维修，构建
累计机器不可用上下文。本入口不传播影响、不修改计划。

## C-SEQ2 第 3 步

`run_stage_cseq2_impact_context.m`

在累计不可用上下文上识别后一故障造成的受影响工序集合，并保留历史维修区间。
本入口不修改机器或 AGV 计划。

## C-SEQ2 第 4 步

`run_stage_cseq2_machine_right_shift.m`
