function scenario = run_stage_b_frozen_problem(baseline)
%RUN_STAGE_B_FROZEN_PROBLEM Build the Stage B complete-search boundary.
...
if nargin < 1
    ...
    scenario = run_stage_b_agv_linked_right_shift(baseline);
end
scenario.frozen_problem = build_stage_b_frozen_problem( ...
    scenario.baseline, scenario.fault, scenario.state, ...
    scenario.resume_plan);
scenario.step = 7;
