function scenario = run_stage_br_complete_search(baseline)
%RUN_STAGE_BR_COMPLETE_SEARCH Run and save the Stage B-R formal search.
...
config = stage_br_complete_search_config(projectRoot);
if nargin < 1
...
scenario.complete_reschedule_search = ...
    search_stage_br_complete_reschedule( ...
...
fprintf('Stage B-R formal complete-rescheduling search completed.\n');
...
if fileId < 0
    error('run_stage_br_complete_search:SummaryFile', ...
...
fprintf(fileId, 'Stage B-R complete-rescheduling formal run\n');
...
fprintf(fileId, 'restart_operation_decoder=1\n');
fprintf(fileId, 'restart_from_zero=%d\n', ...
    commitment.restart_from_zero);
fprintf(fileId, 'progress_preserved=%d\n', ...
    commitment.progress_preserved);
fprintf(fileId, 'lost_processing_time=%.12g\n', ...
    commitment.lost_processing_time);
fprintf(fileId, 'restart_processing_time=%.12g\n', ...
    commitment.original_duration);
fprintf(fileId, 'total_machine_processing_time=%.12g\n', ...
