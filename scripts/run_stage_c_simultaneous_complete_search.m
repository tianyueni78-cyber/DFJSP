function scenario = run_stage_c_simultaneous_complete_search(baseline)
%RUN_STAGE_C_SIMULTANEOUS_COMPLETE_SEARCH Run and save Stage C Step 10.4.
...
config = stage_c_simultaneous_complete_search_config(projectRoot);
if nargin < 1
...
% Block 1: run the approved single-seed formal search.
...
% Block 2: save each run to a new timestamped directory.
...
fprintf(['Stage C simultaneous-fault complete-rescheduling ', ...
    'search completed.\n']);
...
if fileId < 0
    error('run_stage_c_simultaneous_complete_search:SummaryFile', ...
        'Unable to create run summary.');
end
cleanupFile = onCleanup(@() fclose(fileId));

result = scenario.complete_reschedule_search;
commitments = scenario.frozen_problem.interrupted_commitments;
repairs = scenario.frozen_problem.repair_intervals;
fprintf(fileId, ...
    'Stage C simultaneous-fault complete-rescheduling formal run\n');
