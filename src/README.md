...
- `rescheduling/build_stage_c_simultaneous_frozen_problem.m`
  ...
  已扩展为支持一个或多个同组故障，并允许输入当前计划视图。第 16.1 步复用
  它建立连续故障完全重调度冻结问题。
...
- `rescheduling/decode_stage_c_simultaneous_complete_reschedule.m`
  ...
  已扩展为支持一个或多个中断承诺，并允许输入当前计划视图。第 16.2 步复用
  它解码连续故障完全重调度候选。
...
- `rescheduling/search_stage_c_simultaneous_complete_reschedule.m`
  ...
  复用阶段 C 搜索器执行连续故障 `6×2` 轻量搜索契约。搜索目标仍为最终卸载
  完工时间和总能耗，候选由第 16.2 步支持的 Stage C 解码器验证。
...
第 16.6 步不新增 `src/` 核心算法，只记录第 16.4 和第 16.5 的正式结果。

## 阶段 C 第 17.1 步

第 17.1 步不新增 `src/` 核心算法，只定义最终审计场景矩阵配置。

## 阶段 C 第 17.2 步

第 17.2 步不新增 `src/` 核心算法，只复用同时故障和连续故障已验证的搜索、
解码、评价与审计函数，建立最终审计多随机种子运行入口。

## 阶段 C 第 18 步

第 18 步不新增 `src/` 核心算法，只整理阶段 C 最终总结和代码入口导读。

## C-S2 第 1 步

- `rescheduling/build_stage_c_simultaneous_restart_commitments.m`

为同时故障下的多个直接中断工序建立从头加工承诺。承诺记录故障前损失加工段、
修复后完整重加工段、总机器加工时间和不保留进度标志。

## C-S2 第 2 步

- `impact/identify_stage_cs2_restart_affected_operations.m`

从 C-S2 从头加工承诺出发传播影响。传播根完成时间使用 `restart_segment.end`，
然后沿同工件后继和同机器后继传播，并合并多个故障源。

## C-S2 第 3 步

- `rescheduling/build_stage_cs2_machine_right_shift.m`

把 C-S2 损失加工段、完整重加工段和受影响工序投影时间写入机器候选表。
本步只生成机器局部右移候选，不调整 AGV。

## C-S2 第 4 步

- `impact/analyze_stage_cs2_agv_impact.m`

复用阶段 C 同时故障 AGV 影响分析核心，检查 C-S2 从头加工机器候选造成的
运输到达、前序完工、最终卸载和同 AGV 后续复核影响。

## C-S2 第 5 步

- `rescheduling/build_stage_cs2_agv_linked_right_shift.m`

将 C-S2 从头加工承诺映射到阶段 C 同时故障联动核心，正式调整 AGV 时间并
反馈机器开工时间，随后恢复 C-S2 规则标记。

## C-S2 第 6 步

- `rescheduling/build_stage_cs2_frozen_problem.m`

复用阶段 C 同时故障冻结边界结构，并将中断承诺恢复为 C-S2 的从头加工
规则，为后续完全重调度解码器定义固定边界。

## C-S2 第 7 步

- `rescheduling/decode_stage_cs2_complete_reschedule.m`

复用阶段 A 的未开工工序解码核心，并恢复多个从头加工中断承诺。机器能耗
按物理加工段统计，因此损失加工段也计入工作能耗。
...
第 10 步不新增 `src/` 核心算法，只复用第 9 步搜索器并建立正式运行配置、
输出保存和摘要记录入口。

## C-S2 第 11 步

- `evaluation/audit_stage_cs2_rescheduling_candidate.m`

  专门审计同时故障从头加工规则：故障前加工段为损失加工，修复后完整重加工，
  不保留进度，并将损失加工和重加工均纳入机器能耗。

## C-S2 第 12 步

- `evaluation/analyze_stage_cs2_weight_sensitivity.m`

  固定 C-S2 第 11 步候选集，扫描完工时间权重 `0:0.1:1`，记录不同权重下
  被选中的策略、最终卸载时间、`tD`、`SD` 和 `Y`。

## C-SEQ2 第 1 步

- `screening/screen_stage_cseq2_overlapping_next_fault_event.m`

  在当前计划视图中筛选发生于历史维修区间内部的下一故障事件，用于构造
  连续故障且维修区间重叠的场景。

## C-SEQ2 第 2 步

- `fault/build_stage_cseq2_overlapping_unavailability_context.m`

  将历史维修区间和新故障维修区间合并为累计不可用上下文，并记录每个重叠
  关系及其来源事件。

## C-SEQ2 第 3 步

- `impact/build_stage_cseq2_impact_context.m`

  在重叠维修上下文中传播后一故障影响，保留累计不可用区间，并划分受影响与
  未受影响的未开工工序集合。
