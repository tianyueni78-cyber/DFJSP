function analysis = analyze_stage_a_weight_sensitivity( ...
        baseline, state, rightShift, completeSearch, weights)
%ANALYZE_STAGE_A_WEIGHT_SENSITIVITY Re-evaluate fixed candidates by weight.

if nargin < 5
    error('analyze_stage_a_weight_sensitivity:MissingInput', ...
...
if isempty(weights) || any(~isfinite(weights)) || ...
    ...
    error('analyze_stage_a_weight_sensitivity:Weights', ...
...
result = ~isempty(rows) && ...
    all(isfinite([rows.selected_Y])) && ...
    all([rows.selected_SD] >= 0);
if ~result
    error('analyze_stage_a_weight_sensitivity:InvalidResult', ...
