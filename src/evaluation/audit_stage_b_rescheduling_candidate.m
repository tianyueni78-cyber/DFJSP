function audit = audit_stage_b_rescheduling_candidate( ...
        candidate, fault, resumePlan, strategy)
%AUDIT_STAGE_B_RESCHEDULING_CANDIDATE Audit split processing and energy.

if nargin < 4
    error('audit_stage_b_rescheduling_candidate:MissingInput', ...
        'candidate, fault, resumePlan, and strategy are required.');
end
...
splitCommitmentRespected = audit_split_commitment( ...
    candidate.processing_segments, resumePlan);
...
audit.split_commitment_respected = splitCommitmentRespected;
...
audit.is_validated = audit.validation_flags_all_true && ...
    repairRespected && splitCommitmentRespected && ...
...
function result = audit_split_commitment(segments, plan)
...
result = abs(selected(1).start - plan.completed_segment.start) <= 1e-9 && ...
    abs(selected(1).end - plan.completed_segment.end) <= 1e-9 && ...
    abs(selected(2).start - plan.resumed_segment.start) <= 1e-9 && ...
    abs(selected(2).end - plan.resumed_segment.end) <= 1e-9 && ...
    abs(sum([selected.processing_time]) - plan.original_duration) <= 1e-9;
end
...
for index = 1:numel(fields)
    if ~isfield(value, fields{index})
        error('audit_stage_b_rescheduling_candidate:MissingField', ...
