function audit = audit_stage_c_rescheduling_candidate( ...
        candidate, faults, commitments, strategy)
%AUDIT_STAGE_C_RESCHEDULING_CANDIDATE Audit all Stage C constraints.

if nargin < 4
    error('audit_stage_c_rescheduling_candidate:MissingInput', ...
...
commitmentsRespected = audit_commitments( ...
...
audit.interrupted_commitments_respected = commitmentsRespected;
...
audit.is_validated = audit.validation_flags_all_true && ...
    repairRespected && commitmentsRespected && finalUnloadComplete && ...
...
function result = audit_commitments(segments, commitments)
result = true;
for index = 1:numel(commitments)
    ...
    matches = abs(selected(1).start - ...
        plan.completed_segment.start) <= 1e-9 && ...
...
for index = 1:numel(fields)
    if ~isfield(value, fields{index})
        error('audit_stage_c_rescheduling_candidate:MissingField', ...
