...
for index = 1:numel(commitments)
    ...
    matches = plan.restart_from_zero && ~plan.progress_preserved && ...
        ...
        ~contributes_to_completion(selected(1)) && ...
        contributes_to_completion(selected(2)) && ...
...
function result = contributes_to_completion(segment)
if isfield(segment, 'contributes_to_completion')
    result = isequal(segment.contributes_to_completion, true);
    return
end
result = ~strcmp(segment.segment_type, 'lost_processing_before_fault');
end
