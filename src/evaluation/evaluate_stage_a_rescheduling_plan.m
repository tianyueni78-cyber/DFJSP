function evaluation = evaluate_stage_a_rescheduling_plan( ...
        baseline, state, candidate, strategy, weights)
%EVALUATE_STAGE_A_RESCHEDULING_PLAN Calculate tD, SD, and Y.

if nargin < 5
    error('evaluate_stage_a_rescheduling_plan:MissingInput', ...
        'baseline, state, candidate, strategy, and weights are required.');
end
require_fields(baseline, {'makespan', 'isFaultFreeBaseline'}, 'baseline');
require_fields(state, {'unstarted_operations', ...
    'is_validated'}, 'state');
...
evaluation.completion_time_weight = ...
    weights.completion_time_weight;
...
if isfield(candidate, 'makespan') && ...
        isscalar(candidate.makespan) && ...
        isfinite(candidate.makespan)
...
if isempty(unloads) || any(~isfinite([unloads.end]))
    error('evaluate_stage_a_rescheduling_plan:FinalUnloadMissing', ...
...
for index = 1:numel(baselineOperations)
    ...
    if numel(match) ~= 1
        error('evaluate_stage_a_rescheduling_plan:OperationNotUnique', ...
...
if ~baseline.isFaultFreeBaseline || ~state.is_validated
    error('evaluate_stage_a_rescheduling_plan:InvalidInput', ...
        'Validated baseline and state snapshot are required.');
end
if ~isscalar(baseline.makespan) || ~isfinite(baseline.makespan)
    error('evaluate_stage_a_rescheduling_plan:InvalidMakespan', ...
...
if any(~isfinite(values)) || any(values < 0) || ...
    ...
    error('evaluate_stage_a_rescheduling_plan:InvalidWeights', ...
...
for index = 1:numel(fields)
    if ~isfield(value, fields{index})
        error('evaluate_stage_a_rescheduling_plan:MissingField', ...
