function evaluation = evaluate_stage_br_rescheduling_plan( ...
        baseline, state, candidate, strategy, weights)
%EVALUATE_STAGE_BR_RESCHEDULING_PLAN Calculate tD, SD, and Y.
...
if nargin < 5
    error('evaluate_stage_br_rescheduling_plan:MissingInput', ...
...
if isempty(unloads) || any(~isfinite([unloads.end]))
    error('evaluate_stage_br_rescheduling_plan:FinalUnloadMissing', ...
...
for index = 1:numel(baselineOperations)
    ...
    if numel(match) ~= 1
        error('evaluate_stage_br_rescheduling_plan:OperationNotUnique', ...
...
if ~baseline.isFaultFreeBaseline || ~state.is_validated
    error('evaluate_stage_br_rescheduling_plan:InvalidInput', ...
        'Validated baseline and Stage B-R state are required.');
end
if ~isscalar(baseline.makespan) || ~isfinite(baseline.makespan)
    error('evaluate_stage_br_rescheduling_plan:InvalidMakespan', ...
...
if any(~isfinite(values)) || any(values < 0) || ...
    ...
    error('evaluate_stage_br_rescheduling_plan:InvalidWeights', ...
...
for index = 1:numel(fields)
    if ~isfield(value, fields{index})
        error('evaluate_stage_br_rescheduling_plan:MissingField', ...
