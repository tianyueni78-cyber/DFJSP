function selection = select_stage_b_combined_strategy( ...
        baseline, state, rightShift, completeSearch, config)
%SELECT_STAGE_B_COMBINED_STRATEGY Select the Stage B candidate with minimum Y.

if nargin < 5
    error('select_stage_b_combined_strategy:MissingInput', ...
...
if ~isfield(rightShift, 'is_fully_validated') || ...
    ...
    error('select_stage_b_combined_strategy:InvalidRightShift', ...
        'The Stage B AGV-linked right-shift candidate must be validated.');
end
if ~isfield(completeSearch, 'pareto_front') || ...
    ...
    error('select_stage_b_combined_strategy:InvalidSearch', ...
        'A validated Stage B complete-rescheduling front is required.');
end
...
rightEvaluation = evaluate_stage_b_rescheduling_plan( ...
...
template = evaluate_stage_b_rescheduling_plan( ...
...
for index = 1:numel(front)
    evaluations(index) = evaluate_stage_b_rescheduling_plan( ...
...
if selection.selected_metrics.Y > min(scores) + 1e-9
    error('select_stage_b_combined_strategy:SelectionMismatch', ...
...
for index = 1:numel(required)
    if ~isfield(config, required{index})
        error('select_stage_b_combined_strategy:MissingConfig', ...
...
if any(~isfinite(weights)) || any(weights < 0) || ...
    ...
    error('select_stage_b_combined_strategy:InvalidWeights', ...
...
if ~isscalar(config.tie_tolerance) || ...
    ...
    error('select_stage_b_combined_strategy:InvalidTolerance', ...
