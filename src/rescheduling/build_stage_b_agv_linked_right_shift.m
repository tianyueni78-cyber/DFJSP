function candidate = build_stage_b_agv_linked_right_shift( ...
        baseline, fault, machineCandidate, agvImpact)
%BUILD_STAGE_B_AGV_LINKED_RIGHT_SHIFT Couple Stage B AGV and machine times.
...
%   operation keeps its two fixed processing segments; only later work is
%   propagated until transport and processing constraints are feasible.

if nargin < 4
    error('build_stage_b_agv_linked_right_shift:MissingInput', ...
...
require_fields(machineCandidate, {'stage', 'step', ...
    'operation_records', 'resume_plan', 'AGVTable', ...
...
[operations, activities, iterationCount] = propagate_times( ...
    ...
    fault, machineCandidate.resume_plan);

processingSegments = build_processing_segments( ...
    operations, machineCandidate.resume_plan);
...
function [operations, activities, iterationCount] = propagate_times( ...
        operations, activities, operaNumVec, fault, resumePlan)
tolerance = 1e-9;
...
error('build_stage_b_agv_linked_right_shift:NoConvergence', ...
    'Stage B AGV-machine propagation did not converge.');
end
    ...
    for position = 1:numel(indices)
        ...
        if activities(index).is_frozen
            if activities(index).start < previousEnd - 1e-9
                error('build_stage_b_agv_linked_right_shift:FrozenOverlap', ...
...
for orderIndex = 1:numel(operationOrder)
    ...
    if operation.is_interrupted
        validate_interrupted_root(operation, resumePlan);
        continue
    end
...
function validate_interrupted_root(operation, resumePlan)
if operation.job ~= resumePlan.job || ...
        operation.operation ~= resumePlan.operation || ...
        abs(operation.start - resumePlan.original_start) > 1e-9 || ...
        abs(operation.end - resumePlan.revised_completion_time) > 1e-9
    error('build_stage_b_agv_linked_right_shift:InterruptedRootChanged', ...
...
for outputIndex = 1:count
    if isempty(ready)
        error('build_stage_b_agv_linked_right_shift:OperationCycle', ...
...
for jobId = 1:numel(operaNumVec)
    ...
    for operationId = 1:operaNumVec(jobId)
        ...
        if numel(match) ~= 1
            error('build_stage_b_agv_linked_right_shift:OperationNotUnique', ...
...
if numel(position) ~= 1
    error('build_stage_b_agv_linked_right_shift:MachineOrder', ...
...
elseif numel(match) == 1
    ...
    error('build_stage_b_agv_linked_right_shift:TransportNotUnique', ...
...
function segments = build_processing_segments(operations, resumePlan)
...
for index = 1:numel(operations)
    ...
    if operation.is_interrupted
        segments(end + 1) = segment_from_plan( ...
            operation, resumePlan.completed_segment, 1);
        segments(end + 1) = segment_from_plan( ...
            operation, resumePlan.resumed_segment, 2);
    else
        segment = template;
...
function validation = validate_candidate(operations, segments, ...
        activities, operaNumVec, fault, resumePlan)
tolerance = 1e-9;
...
validate_interrupted_segments(segments, resumePlan, tolerance);
...
for machineId = unique([segments.machine_id])
    ...
    for index = 1:numel(selected) - 1
        if selected(index).end > selected(index + 1).start + tolerance
            error('build_stage_b_agv_linked_right_shift:MachineOverlap', ...
...
for index = 1:numel(failed)
    ...
    if overlaps
        error('build_stage_b_agv_linked_right_shift:RepairOverlap', ...
    ...
    for operationId = 1:operaNumVec(jobId)
        ...
        if operationId > 1
            ...
            if predecessor.end > operation.start + tolerance
                error('build_stage_b_agv_linked_right_shift:JobPrecedence', ...
        ...
        if ~isempty(arrival) && arrival > operation.start + tolerance
            error('build_stage_b_agv_linked_right_shift:LateTransport', ...
...
for agvId = unique([activities.agv_id])
    ...
    for index = 1:numel(records) - 1
        if records(index).end > records(index + 1).start + tolerance
            error('build_stage_b_agv_linked_right_shift:AGVOverlap', ...
...
for index = 1:numel(activities)
    ...
    if activity.is_frozen && ...
        ...
        error('build_stage_b_agv_linked_right_shift:FrozenActivityChanged', ...
    ...
    if activity.load_status == -2 && activity.job > 0
        ...
        if activity.start < ready - tolerance
            error('build_stage_b_agv_linked_right_shift:EarlyTransport', ...
...
function validate_interrupted_segments(segments, resumePlan, tolerance)
selected = segments([segments.job] == resumePlan.job & ...
    [segments.operation] == resumePlan.operation);
if numel(selected) ~= 2
    error('build_stage_b_agv_linked_right_shift:InterruptedSegments', ...
...
if abs(selected(1).start - resumePlan.completed_segment.start) > ...
        ...
        tolerance || ...
        abs(selected(2).end - resumePlan.resumed_segment.end) > tolerance
    error('build_stage_b_agv_linked_right_shift:InterruptedSegments', ...
...
if ~baseline.isFaultFreeBaseline || ~fault.is_validated || ...
        ...
        ~strcmp(machineCandidate.stage, 'B') || ...
        machineCandidate.step ~= 4 || ...
        ~strcmp(agvImpact.stage, 'B') || agvImpact.step ~= 5
    error('build_stage_b_agv_linked_right_shift:InvalidInput', ...
        'Validated Stage B Steps 4 and 5 are required.');
end
if ~isequaln(machineCandidate.AGVTable, baseline.AGVTable)
    error('build_stage_b_agv_linked_right_shift:AGVTableChanged', ...
...
for index = 1:numel(fields)
    if ~isfield(value, fields{index})
        error('build_stage_b_agv_linked_right_shift:MissingField', ...
