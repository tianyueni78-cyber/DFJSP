function candidate = build_stage_br_agv_linked_right_shift( ...
        baseline, fault, machineCandidate, agvImpact)
%BUILD_STAGE_BR_AGV_LINKED_RIGHT_SHIFT Couple Stage B-R AGV and machine times.
%   Machine and AGV assignments, routes, sequence, and durations remain
%   unchanged. Completed AGV activities are frozen. The interrupted
%   operation keeps the lost pre-fault segment and the full restart
%   segment; only later work is propagated until all constraints hold.

if nargin < 4
    error('build_stage_br_agv_linked_right_shift:MissingInput', ...
...
require_fields(machineCandidate, {'stage', 'step', ...
    'operation_records', 'restart_plan', 'AGVTable', ...
    'restart_from_zero', 'lost_processing_time', ...
...
validation = validate_candidate(operations, processingSegments, ...
    activities, baseline.problem.operaNumVec, fault, ...
    machineCandidate.restart_plan);
...
candidate.lost_processing_time = ...
    machineCandidate.restart_plan.lost_processing_time;
...
function [operations, activities, iterationCount] = propagate_times( ...
        operations, activities, operaNumVec, fault, restartPlan)
tolerance = 1e-9;
...
error('build_stage_br_agv_linked_right_shift:NoConvergence', ...
    'Stage B-R AGV-machine propagation did not converge.');
end
    ...
    for position = 1:numel(indices)
        ...
        if activities(index).is_frozen
            if activities(index).start < previousEnd - 1e-9
                error('build_stage_br_agv_linked_right_shift:FrozenOverlap', ...
...
for orderIndex = 1:numel(operationOrder)
    ...
    if operation.is_interrupted
        validate_interrupted_root(operation, restartPlan);
        continue
    end
    ...
    if operation.machine_id == fault.machine_id && ...
            ...
            earliest + operation.processing_duration > fault.start_time
        earliest = fault.repair_end_time;
    end
...
function validate_interrupted_root(operation, restartPlan)
if operation.job ~= restartPlan.job || ...
        operation.operation ~= restartPlan.operation || ...
        abs(operation.start - restartPlan.original_start) > 1e-9 || ...
        abs(operation.end - restartPlan.revised_completion_time) > 1e-9
    error('build_stage_br_agv_linked_right_shift:InterruptedRootChanged', ...
...
for outputIndex = 1:count
    if isempty(ready)
        error('build_stage_br_agv_linked_right_shift:OperationCycle', ...
...
for jobId = 1:numel(operaNumVec)
    ...
    for operationId = 1:operaNumVec(jobId)
        ...
        if numel(match) ~= 1
            error('build_stage_br_agv_linked_right_shift:OperationNotUnique', ...
...
if numel(position) ~= 1
    error('build_stage_br_agv_linked_right_shift:MachineOrder', ...
...
elseif numel(match) == 1
    ...
    error('build_stage_br_agv_linked_right_shift:TransportNotUnique', ...
...
function segments = build_processing_segments(operations, restartPlan)
...
for index = 1:numel(operations)
    ...
    if operation.is_interrupted
        segments(end + 1) = segment_from_plan( ...
            operation, restartPlan.lost_processing_segment, 1);
        segments(end + 1) = segment_from_plan( ...
            operation, restartPlan.restart_segment, 2);
    else
        segment = template;
...
function validation = validate_candidate(operations, segments, ...
        activities, operaNumVec, fault, restartPlan)
tolerance = 1e-9;
validate_machine_constraints(segments, fault, tolerance);
...
validate_interrupted_segments(segments, restartPlan, tolerance);
...
function validate_machine_constraints(segments, fault, tolerance)
for machineId = unique([segments.machine_id])
    ...
    for index = 1:numel(selected) - 1
        if selected(index).end > selected(index + 1).start + tolerance
            error('build_stage_br_agv_linked_right_shift:MachineOverlap', ...
...
for index = 1:numel(failed)
    overlaps = failed(index).start < fault.repair_end_time - tolerance && ...
        fault.start_time < failed(index).end - tolerance;
    if overlaps
        error('build_stage_br_agv_linked_right_shift:RepairOverlap', ...
            'Failed machine processes during repair.');
    end
end
end
    ...
    for operationId = 1:operaNumVec(jobId)
        ...
        if operationId > 1
            ...
            if predecessor.end > operation.start + tolerance
                error('build_stage_br_agv_linked_right_shift:JobPrecedence', ...
        ...
        if ~isempty(arrival) && arrival > operation.start + tolerance
            error('build_stage_br_agv_linked_right_shift:LateTransport', ...
...
for agvId = unique([activities.agv_id])
    ...
    for index = 1:numel(records) - 1
        if records(index).end > records(index + 1).start + tolerance
            error('build_stage_br_agv_linked_right_shift:AGVOverlap', ...
...
for index = 1:numel(activities)
    ...
    if activity.is_frozen && ...
        ...
        error('build_stage_br_agv_linked_right_shift:FrozenActivityChanged', ...
    ...
    if activity.load_status == -2 && activity.job > 0
        ...
        if activity.start < ready - tolerance
            error('build_stage_br_agv_linked_right_shift:EarlyTransport', ...
...
function validate_interrupted_segments(segments, restartPlan, tolerance)
selected = segments([segments.job] == restartPlan.job & ...
    [segments.operation] == restartPlan.operation);
if numel(selected) ~= 2
    error('build_stage_br_agv_linked_right_shift:InterruptedSegments', ...
...
function validate_inputs(baseline, fault, machineCandidate, agvImpact)
if ~baseline.isFaultFreeBaseline || ~fault.is_validated || ...
        ...
        ~strcmp(machineCandidate.stage, 'B-R') || ...
        machineCandidate.step ~= 3 || ...
        ~strcmp(agvImpact.stage, 'B-R') || agvImpact.step ~= 4 || ...
        ~machineCandidate.restart_from_zero || ...
        ~agvImpact.restart_from_zero || agvImpact.progress_preserved || ...
        ~strcmp(machineCandidate.restart_plan.rule, ...
        'restart_on_original_machine')
    error('build_stage_br_agv_linked_right_shift:InvalidInput', ...
        'Validated Stage B-R Steps 3 and 4 are required.');
end
if ~isequaln(machineCandidate.AGVTable, baseline.AGVTable)
    error('build_stage_br_agv_linked_right_shift:AGVTableChanged', ...
...
for index = 1:numel(fields)
    if ~isfield(value, fields{index})
        error('build_stage_br_agv_linked_right_shift:MissingField', ...
