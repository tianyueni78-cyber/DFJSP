function candidate = build_stage_cs2_machine_right_shift( ...
        baseline, faults, state, restartCommitments, impact)
%BUILD_STAGE_CS2_MACHINE_RIGHT_SHIFT Build C-S2 machine candidate.
%   Interrupted operations lose their pre-fault progress and restart on
%   the original machine for their full original durations after repair.
%   Impact-projected successor times are written into a copied machine
%   schedule. AGV tasks remain unchanged in this machine-only step.

if nargin < 5
    error('build_stage_cs2_machine_right_shift:MissingInput', ...
        'baseline, faults, state, restartCommitments, and impact are required.');
end
validate_inputs(baseline, faults, state, restartCommitments, impact);

records = collect_operations(baseline.machineTable);
records = apply_restart_commitments(records, restartCommitments);
records = apply_affected_times(records, impact.affected_operations, ...
    restartCommitments);
segments = build_processing_segments(records, restartCommitments);
machineTable = rebuild_machine_table(segments, numel(baseline.machineTable));
validation = validate_candidate(baseline, faults, impact, ...
    restartCommitments, records, segments, machineTable);
...
candidate.interrupted_commitments = restartCommitments;
candidate.unavailable_intervals = build_unavailable_intervals(faults);
candidate.lost_processing_time = sum([restartCommitments.lost_processing_time]);
...
function records = apply_restart_commitments(records, commitments)
for index = 1:numel(commitments)
    commitment = commitments(index);
    match = find_operation(records, commitment.job, ...
        commitment.operation);
    if records(match).machine_id ~= commitment.machine_id
        error('build_stage_cs2_machine_right_shift:InterruptedMachineChanged', ...
...
function records = apply_affected_times(records, affected, commitments)
for index = 1:numel(affected)
    ...
    if records(match).machine_id ~= source.machine_id
        error('build_stage_cs2_machine_right_shift:MachineChanged', ...
            'Partial right shift cannot change machine assignment.');
    end
    if is_commitment(commitments, source.job, source.operation)
        error('build_stage_cs2_machine_right_shift:RootDuplicated', ...
...
function result = is_commitment(commitments, job, operation)
result = any([commitments.job] == job & ...
...
for index = 1:numel(records)
    ...
    if record.is_interrupted
        ...
        segments(end + 1) = segment_from_commitment( ...
            commitment.lost_processing_segment, 1);
        segments(end + 1) = segment_from_commitment( ...
            commitment.restart_segment, 2);
        continue
    end
    segment = template;
...
function segment = segment_from_commitment(source, segmentOrder)
segment = segment_template();
segment.machine_id = source.machine_id;
segment.original_table_index = source.original_table_index;
...
matches = find([commitments.job] == job & ...
    [commitments.operation] == operation);
if numel(matches) ~= 1
    error('build_stage_cs2_machine_right_shift:CommitmentNotUnique', ...
...
validate_restart_commitments(commitments, records, segments, tolerance);
...
validation.restart_commitments_respected = true;
...
if numel(source) ~= numel(records)
    error('build_stage_cs2_machine_right_shift:OperationCountChanged', ...
...
for index = 1:numel(source)
    ...
    if records(match).machine_id ~= source(index).machine_id || ...
        ...
        error('build_stage_cs2_machine_right_shift:OperationIdentityChanged', ...
            'Machine assignment or logical processing duration changed.');
    end
end
end

function validate_restart_commitments(commitments, records, segments, ...
        tolerance)
for index = 1:numel(commitments)
    commitment = commitments(index);
    match = find_operation(records, commitment.job, ...
    ...
    if ~records(match).is_interrupted || ...
            ~records(match).is_restart_from_zero || ...
            abs(records(match).end - ...
            commitment.revised_completion_time) > tolerance || ...
            abs(records(match).lost_processing_time - ...
            commitment.lost_processing_time) > tolerance
        error('build_stage_cs2_machine_right_shift:CommitmentTimeMismatch', ...
            'Interrupted logical operation violates its restart commitment.');
    end
    selected = segments([segments.job] == commitment.job & ...
        [segments.operation] == commitment.operation);
    if numel(selected) ~= 2
        error('build_stage_cs2_machine_right_shift:CommitmentSegmentCount', ...
            'Interrupted operation must contain two segments.');
    end
    ...
    if abs(selected(1).start - ...
        ...
        error('build_stage_cs2_machine_right_shift:CommitmentSegmentMismatch', ...
            'Interrupted processing segments do not match restart commitment.');
    end
end
end
...
for index = 1:numel(records)
    record = records(index);
    if is_commitment(commitments, record.job, record.operation)
        continue
    end
    ...
    if ~valid
        error('build_stage_cs2_machine_right_shift:OperationTimeMismatch', ...
...
for machineId = 1:machineCount
    ...
    for index = 1:numel(selected) - 1
        if selected(index).end > selected(index + 1).start + tolerance
            error('build_stage_cs2_machine_right_shift:MachineOverlap', ...
...
for machineId = 1:numel(machineTable)
    ...
    if isempty(blocks) || abs(blocks(1).start) > tolerance || ...
        ...
        error('build_stage_cs2_machine_right_shift:InvalidMachineTable', ...
...
for job = 1:numel(operaNumVec)
    for operation = 1:operaNumVec(job) - 1
        ...
        if current.end > successor.start + tolerance
            error('build_stage_cs2_machine_right_shift:JobPrecedence', ...
...
for faultIndex = 1:numel(faults)
    ...
    for index = 1:numel(selected)
        ...
        if overlaps
            error('build_stage_cs2_machine_right_shift:RepairOverlap', ...
...
if numel(index) ~= 1
    error('build_stage_cs2_machine_right_shift:OperationNotUnique', ...
        'Operation J%d-O%d must appear exactly once.', job, operation);
end
end

function validate_inputs(baseline, faults, state, commitments, impact)
...
if (~isFaultFree && ~isCurrentView) || ~state.is_validated || ...
    ...
    error('build_stage_cs2_machine_right_shift:InvalidInput', ...
        'Validated C-S2 Step 2 inputs are required.');
end
end
...
value = struct('machine_id', [], 'original_table_index', [], ...
    ...
    'operation', [], 'start', [], 'end', [], ...
    'processing_time', [], 'contributes_to_completion', true, ...
    'source_event_ids', []);
end
...
for index = 1:numel(fields)
    if ~isfield(value, fields{index})
        error('build_stage_cs2_machine_right_shift:MissingField', ...
