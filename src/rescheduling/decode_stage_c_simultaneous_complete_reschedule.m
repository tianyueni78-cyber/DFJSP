function candidate = decode_stage_c_simultaneous_complete_reschedule( ...
        baseline, frozen, decision)
%DECODE_STAGE_C_SIMULTANEOUS_COMPLETE_RESCHEDULE Decode Stage C Step 10.1.
...
if nargin < 3
    error('decode_stage_c_simultaneous_complete_reschedule:MissingInput', ...
...
machineEnergy = calculate_machine_energy(operations, baseline);

% Block 3: audit the complete candidate against every Stage C constraint.
validation = validate_stage_c_candidate( ...
    operations, segments, coreCandidate, frozen, baseline.problem);

candidate = coreCandidate;
candidate.stage = 'C';
candidate.step = 10;
candidate.substep = '10.1';
candidate.decoder = 'stage_c_multiple_split_operation_decoder';
...
for index = 1:numel(commitments)
    ...
    if numel(match) ~= 1
        error('decode_stage_c_simultaneous_complete_reschedule:InterruptedNotUnique', ...
    ...
    if operation.machine_id ~= commitment.machine_id || ...
        ...
        error('decode_stage_c_simultaneous_complete_reschedule:InterruptedChanged', ...
...
if numel(match) ~= 1
    error('decode_stage_c_simultaneous_complete_reschedule:CommitmentNotUnique', ...
...
for machineId = 1:machineCount
    ...
    if idle(machineId) < -1e-9
        error('decode_stage_c_simultaneous_complete_reschedule:NegativeIdle', ...
...
function validation = validate_stage_c_candidate( ...
...
validation.stage_c_machine_energy_recalculated = true;
validation.stage_c_decoder_contract = true;
end

function validate_operation_partition(operations, operaNumVec)
if numel(operations) ~= sum(operaNumVec)
    error('decode_stage_c_simultaneous_complete_reschedule:OperationCount', ...
...
for jobId = 1:numel(operaNumVec)
    for operationId = 1:operaNumVec(jobId)
        ...
        if numel(match) ~= 1
            error('decode_stage_c_simultaneous_complete_reschedule:OperationPartition', ...
...
for index = 1:numel(frozen.frozen_operations)
    ...
    if target.machine_id ~= source.machine_id || ...
        ...
        error('decode_stage_c_simultaneous_complete_reschedule:FrozenChanged', ...
...
if interruptedCount ~= numel(commitments)
    error('decode_stage_c_simultaneous_complete_reschedule:InterruptedCount', ...
...
for index = 1:numel(commitments)
    ...
    if ~operation.is_interrupted || ...
        ...
        error('decode_stage_c_simultaneous_complete_reschedule:InterruptedMismatch', ...
    ...
    if numel(selected) ~= 2
        error('decode_stage_c_simultaneous_complete_reschedule:SegmentCount', ...
    ...
    if abs(selected(1).start - commitment.completed_segment.start) > ...
        ...
        error('decode_stage_c_simultaneous_complete_reschedule:SegmentMismatch', ...
...
for machineId = 1:machineCount
    ...
    for index = 1:numel(records) - 1
        if records(index).end > records(index + 1).start + tolerance
            error('decode_stage_c_simultaneous_complete_reschedule:MachineOverlap', ...
...
for intervalIndex = 1:numel(intervals)
    ...
    for recordIndex = 1:numel(records)
        ...
        if overlaps
            error('decode_stage_c_simultaneous_complete_reschedule:RepairOverlap', ...
...
for jobId = 1:numel(operaNumVec)
    for operationId = 1:operaNumVec(jobId) - 1
        ...
        if current.end > successor.start + tolerance
            error('decode_stage_c_simultaneous_complete_reschedule:JobPrecedence', ...
...
if ~candidate.is_validated || ...
    ...
    error('decode_stage_c_simultaneous_complete_reschedule:InvalidCoreCandidate', ...
...
if numel(match) ~= 1
    error('decode_stage_c_simultaneous_complete_reschedule:OperationNotUnique', ...
        'J%d-O%d must appear exactly once.', jobId, operationId);
end
...
if ~baseline.isFaultFreeBaseline || ~frozen.is_validated || ...
        ~strcmp(frozen.stage, 'C') || frozen.step ~= 9 || ...
        frozen.stage_a_decoder_compatible || ...
        numel(frozen.interrupted_commitments) < 2 || ...
        ~strcmp(frozen.decoder_requirement, ...
        'stage_c_multiple_split_operation_decoder')
    error('decode_stage_c_simultaneous_complete_reschedule:InvalidInput', ...
        'Validated Stage C Step 9 inputs are required.');
end
end
...
for index = 1:numel(fields)
    if ~isfield(value, fields{index})
        error('decode_stage_c_simultaneous_complete_reschedule:MissingField', ...
