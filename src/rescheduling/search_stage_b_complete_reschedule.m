function result = search_stage_b_complete_reschedule( ...
        baseline, frozen, options)
%SEARCH_STAGE_B_COMPLETE_RESCHEDULE Run Stage B restricted NSGA-II.
%   This search operates only on unstarted-operation decisions and uses
%   the split-operation decoder for makespan and total-energy objectives.

if nargin < 3
    error('search_stage_b_complete_reschedule:MissingInput', ...
...
result.is_split_operation_evaluated = true;
...
for index = 1:numel(decisions)
    value = evaluate_stage_b_reschedule_candidate( ...
...
if numel(searchResult.history) ~= ...
    ...
    error('search_stage_b_complete_reschedule:HistoryLength', ...
        'Search history length is incorrect.');
end
if searchResult.completed_generations > options.generations
    error('search_stage_b_complete_reschedule:GenerationLimit', ...
...
if size(unique(frontObjectives, 'rows'), 1) ~= ...
    ...
    error('search_stage_b_complete_reschedule:DuplicatePareto', ...
...
for index = 1:numel(required)
    if ~isfield(options, required{index})
        error('search_stage_b_complete_reschedule:MissingOption', ...
...
if ~isscalar(options.improvement_tolerance) || ...
    ...
    error('search_stage_b_complete_reschedule:InvalidOption', ...
...
value = struct('decision', [], 'candidate', [], ...
    ...
    'is_split_operation_evaluated', false, ...
    'is_validated', false);
end
