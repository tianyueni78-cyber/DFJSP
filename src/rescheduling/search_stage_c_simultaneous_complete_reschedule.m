function result = search_stage_c_simultaneous_complete_reschedule( ...
        baseline, frozen, options)
%SEARCH_STAGE_C_SIMULTANEOUS_COMPLETE_RESCHEDULE Run Stage C simultaneous-fault restricted NSGA-II.
%   This search operates only on unstarted-operation decisions and uses
%   the multiple-interruption decoder for makespan and total-energy objectives.

if nargin < 3
    error('search_stage_c_simultaneous_complete_reschedule:MissingInput', ...
...
result.is_multiple_interruption_evaluated = true;
result.are_all_repair_intervals_evaluated = true;
...
for index = 1:numel(decisions)
    value = evaluate_stage_c_simultaneous_reschedule_candidate( ...
...
if numel(searchResult.population) ~= options.population_size
    error('search_stage_c_simultaneous_complete_reschedule:PopulationSize', ...
        'Final population size is incorrect.');
end
if isempty(searchResult.pareto_front) || ...
        any(~[searchResult.population.is_validated])
    error('search_stage_c_simultaneous_complete_reschedule:InvalidResult', ...
...
if numel(searchResult.history) ~= ...
    ...
    error('search_stage_c_simultaneous_complete_reschedule:HistoryLength', ...
        'Search history length is incorrect.');
end
if searchResult.completed_generations > options.generations
    error('search_stage_c_simultaneous_complete_reschedule:GenerationLimit', ...
...
if size(unique(frontObjectives, 'rows'), 1) ~= ...
    ...
    error('search_stage_c_simultaneous_complete_reschedule:DuplicatePareto', ...
...
for index = 1:numel(required)
    if ~isfield(options, required{index})
        error('search_stage_c_simultaneous_complete_reschedule:MissingOption', ...
...
if ~isscalar(options.improvement_tolerance) || ...
    ...
    error('search_stage_c_simultaneous_complete_reschedule:InvalidOption', ...
...
value = struct('decision', [], 'candidate', [], ...
    ...
    'is_final_unload_evaluated', false, ...
    'interrupted_operation_count', [], ...
    'repair_interval_count', [], ...
    'is_multiple_interruption_evaluated', false, ...
    'are_all_repair_intervals_evaluated', false, ...
    'is_validated', false);
end
