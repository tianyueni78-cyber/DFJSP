function result = search_stage_cs2_complete_reschedule( ...
        baseline, frozen, options)
%search_stage_cs2_complete_reschedule Run C-S2 restart-from-zero restricted NSGA-II.
%   This search operates only on unstarted-operation decisions and uses
%   the multiple-restart decoder for makespan and total-energy objectives.

if nargin < 3
    error('search_stage_cs2_complete_reschedule:MissingInput', ...
...
result = struct();
result.stage = 'C-S2';
result.strategy = 'cs2_restricted_complete_rescheduling_nsga2';
...
result.is_multiple_restart_operation_evaluated = true;
...
for index = 1:numel(decisions)
    value = evaluate_stage_cs2_reschedule_candidate( ...
...
if numel(searchResult.population) ~= options.population_size
    error('search_stage_cs2_complete_reschedule:PopulationSize', ...
        'Final population size is incorrect.');
end
if isempty(searchResult.pareto_front) || ...
        any(~[searchResult.population.is_validated])
    error('search_stage_cs2_complete_reschedule:InvalidResult', ...
...
if numel(searchResult.history) ~= ...
    ...
    error('search_stage_cs2_complete_reschedule:HistoryLength', ...
        'Search history length is incorrect.');
end
if searchResult.completed_generations > options.generations
    error('search_stage_cs2_complete_reschedule:GenerationLimit', ...
...
if size(unique(frontObjectives, 'rows'), 1) ~= ...
    ...
    error('search_stage_cs2_complete_reschedule:DuplicatePareto', ...
...
for index = 1:numel(required)
    if ~isfield(options, required{index})
        error('search_stage_cs2_complete_reschedule:MissingOption', ...
...
if ~isscalar(options.improvement_tolerance) || ...
    ...
    error('search_stage_cs2_complete_reschedule:InvalidOption', ...
...
value = struct('decision', [], 'candidate', [], ...
    ...
    'is_multiple_restart_operation_evaluated', false, ...
