...
contractScenario = run_stage_a_combination_contract();
selection = contractScenario.combined_selection;

assert(selection.is_validated);
assert(contractScenario.is_combination_evaluated);
...
assert(numel(selection.evaluations) == ...
    1 + numel(contractScenario.complete_reschedule_search.pareto_front));
...
for index = 1:numel(selection.evaluations)
    ...
    expectedTD = evaluation.candidate_makespan - ...
        contractScenario.baseline.makespan;
...
fprintf('test_stage_a_combination_contract passed\n');
