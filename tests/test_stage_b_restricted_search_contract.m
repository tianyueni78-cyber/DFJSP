...
first = run_stage_b_restricted_search_contract();
second = run_stage_b_restricted_search_contract();
...
assert(all([result.population.is_split_operation_evaluated]));
...
for index = 1:numel(result.population)
    ...
    assert(evaluation.candidate.is_stage_b_split_operation_decoded);
...
adaptive = search_stage_b_complete_reschedule( ...
...
rng(first.baseline.seed);
timeLimited = search_stage_b_complete_reschedule( ...
...
fprintf('test_stage_b_restricted_search_contract passed\n');
