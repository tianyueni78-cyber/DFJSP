...
first = run_stage_br_restricted_search_contract();
second = run_stage_br_restricted_search_contract();
...
assert(result.is_restart_operation_evaluated);
...
assert(all([result.population.is_restart_operation_evaluated]));
assert(all([result.population.restart_from_zero]));
...
expectedLostProcessing = ...
    first.frozen_problem.interrupted_commitment.lost_processing_time;
for index = 1:numel(result.population)
    ...
    assert(evaluation.candidate.is_stage_br_restart_operation_decoded);
    assert(evaluation.restart_from_zero);
    assert(evaluation.lost_processing_time > 0);
    assert(abs(evaluation.lost_processing_time - ...
        expectedLostProcessing) <= 1e-9);
...
adaptive = search_stage_br_complete_reschedule( ...
...
rng(first.baseline.seed);
timeLimited = search_stage_br_complete_reschedule( ...
...
fprintf('test_stage_br_restricted_search_contract passed\n');
