...
scenario = run_stage_c_sequential_restricted_search_contract();
scenario.sequential_complete_reschedule_search = ...
    scenario.sequential_restricted_search;
combined = run_stage_c_sequential_combination_selection(scenario);
...
assert(combined.step == 16);
assert(strcmp(combined.substep, '16.5'));
...
assert(numel(selection.evaluations) == ...
    1 + numel(combined.sequential_complete_reschedule_search.pareto_front));
...
fprintf('test_stage_c_sequential_combination_contract passed\n');
