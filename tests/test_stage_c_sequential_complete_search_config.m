...
assert(strcmp(config.run_type, ...
    'single_seed_sequential_formal_search'));
assert(strcmp(config.output_root, fullfile(projectRoot, ...
    'outputs', ...
    'stage_c_sequential_complete_reschedule_search')));
assert(exist('run_stage_c_sequential_complete_search', 'file') == 2);
assert(~exist(config.output_root, 'dir') || ...
    isfolder(config.output_root));

fprintf('test_stage_c_sequential_complete_search_config passed\n');
