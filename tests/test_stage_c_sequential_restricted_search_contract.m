...
first = run_stage_c_sequential_restricted_search_contract();
second = run_stage_c_sequential_restricted_search_contract();
result = first.sequential_restricted_search;
frozen = first.sequential_frozen_problem;

assert(first.is_validated);
assert(first.step == 16);
assert(strcmp(first.substep, '16.3b'));
...
firstObjectives = reshape( ...
    [first.sequential_restricted_search.population.objectives], 2, []).';
secondObjectives = reshape( ...
    [second.sequential_restricted_search.population.objectives], 2, []).';
...
adaptive = search_stage_c_simultaneous_complete_reschedule( ...
    first.next_fault_state.current_plan_view, frozen, adaptiveOptions);
...
timeLimited = search_stage_c_simultaneous_complete_reschedule( ...
    first.next_fault_state.current_plan_view, frozen, timeOptions);
...
fprintf('test_stage_c_sequential_restricted_search_contract passed\n');
