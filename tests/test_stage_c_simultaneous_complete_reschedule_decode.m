...
scenario = run_stage_c_simultaneous_complete_reschedule_decode();
candidate = scenario.complete_reschedule_candidate;
frozen = scenario.frozen_problem;
tolerance = 1e-9;

assert(scenario.is_validated);
assert(scenario.step == 10);
assert(strcmp(scenario.substep, '10.1'));
...
fprintf(['test_stage_c_simultaneous_complete_reschedule_decode ', ...
    'passed\n']);
