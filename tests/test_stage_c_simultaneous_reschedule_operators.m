...
first = run_stage_c_simultaneous_reschedule_operators();
second = run_stage_c_simultaneous_reschedule_operators();

assert(first.is_validated);
assert(first.step == 10);
assert(strcmp(first.substep, '10.2'));
...
fprintf('test_stage_c_simultaneous_reschedule_operators passed\n');
...
expectedSegmentCount = ...
    ...
    numel(scenario.frozen_problem.interrupted_commitments);
