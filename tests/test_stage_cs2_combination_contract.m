...
scenario = run_stage_cs2_combination_contract();
selection = scenario.combined_selection;

assert(scenario.is_validated);
assert(strcmp(scenario.step, 'C-S2.11'));
assert(strcmp(scenario.substep, '11'));
...
assert(scenario.right_shift_audit.restart_commitments_respected);
assert(scenario.right_shift_audit.restart_from_zero);
assert(~scenario.right_shift_audit.progress_preserved);
assert(scenario.right_shift_audit.lost_processing_time > 0);
...
assert(all([scenario.complete_reschedule_audits.restart_from_zero]));
assert(~any([scenario.complete_reschedule_audits.progress_preserved]));
assert(scenario.all_constraint_audits_validated);
assert(scenario.all_energy_audits_complete);

fprintf('test_stage_cs2_combination_contract passed\n');
