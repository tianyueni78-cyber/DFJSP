...
first = run_stage_cs2_restricted_search_contract();
second = run_stage_cs2_restricted_search_contract();
result = first.restricted_search;

assert(first.is_validated);
assert(strcmp(first.step, 'C-S2.9'));
...
assert(result.is_multiple_restart_operation_evaluated);
...
assert(all([result.population.is_multiple_restart_operation_evaluated]));
...
expectedInterrupted = ...
    numel(first.cs2_frozen_problem.interrupted_commitments);
expectedIntervals = numel(first.cs2_frozen_problem.repair_intervals);
for index = 1:numel(result.population)
    ...
    assert(evaluation.objectives(1) >= ...
        first.cs2_frozen_problem.snapshot_time);
    ...
    assert(evaluation.candidate.is_stage_cs2_multiple_restart_operation_decoded);
    assert(evaluation.restart_from_zero);
    assert(~evaluation.progress_preserved);
    assert(evaluation.lost_processing_time > 0);
    assert(evaluation.total_machine_processing_time > ...
...
adaptive = search_stage_cs2_complete_reschedule( ...
    first.baseline, first.cs2_frozen_problem, adaptiveOptions);
...
rng(first.baseline.seed);
timeLimited = search_stage_cs2_complete_reschedule( ...
    first.baseline, first.cs2_frozen_problem, timeOptions);
...
fprintf(['test_stage_cs2_restricted_search_contract ', ...
    'passed\n']);
