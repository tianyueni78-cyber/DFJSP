...
scenario = run_stage_cseq2_complete_reschedule_decode();
candidate = scenario.cseq2_complete_reschedule_candidate;
frozen = scenario.cseq2_frozen_problem;
tolerance = 1e-9;

assert(scenario.is_validated);
assert(strcmp(scenario.step, 'C-SEQ2.8'));
assert(strcmp(scenario.substep, '8'));
assert(scenario.is_complete_reschedule_decoded);
assert(~scenario.is_search_executed_in_cseq2_step_8);
assert(~scenario.is_combination_evaluated);
assert(candidate.is_validated);
assert(strcmp(candidate.stage, 'C-SEQ2'));
assert(strcmp(candidate.step, '8'));
...
assert(numel(candidate.active_previous_repairs) > 0);
...
assert_cumulative_repair_intervals(candidate.processing_segments, ...
    frozen.cumulative_unavailability, frozen.frozen_operations, tolerance);
...
fprintf('test_stage_cseq2_complete_reschedule_decode passed\n');
...
function assert_cumulative_repair_intervals( ...
        segments, cumulative, frozenOperations, tolerance)
assert(cumulative.is_validated);
for intervalIndex = 1:numel(cumulative.intervals)
    interval = cumulative.intervals(intervalIndex);
    records = segments([segments.machine_id] == interval.machine_id);
    for recordIndex = 1:numel(records)
        overlaps = records(recordIndex).start < ...
            interval.end_time - tolerance && ...
            records(recordIndex).end > interval.start_time + tolerance;
        if overlaps
            assert(is_frozen_operation(records(recordIndex), frozenOperations));
        end
    end
end
end
