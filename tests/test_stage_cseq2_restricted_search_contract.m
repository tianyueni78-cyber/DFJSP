...
first = run_stage_cseq2_restricted_search_contract();
second = run_stage_cseq2_restricted_search_contract();
result = first.cseq2_restricted_search;
...
assert(strcmp(first.step, 'C-SEQ2.10'));
assert(strcmp(first.substep, '10'));
assert(first.is_operator_contract_built);
assert(first.is_fitness_evaluated);
assert(first.is_search_executed_in_cseq2_step_10);
assert(~first.is_full_experiment);
...
assert(result.is_cseq2_search_contract);
...
assert(result.cumulative_unavailability.is_validated);
...
firstObjectives = reshape( ...
    [first.cseq2_restricted_search.population.objectives], 2, []).';
secondObjectives = reshape( ...
    [second.cseq2_restricted_search.population.objectives], 2, []).';
assert(isequal(firstObjectives, secondObjectives));

expectedInterrupted = numel(frozen.interrupted_commitments);
expectedIntervals = numel(frozen.repair_intervals);
for index = 1:numel(result.population)
    ...
    assert(evaluation.objectives(1) >= frozen.snapshot_time);
    assert(evaluation.objectives(2) >= 0);
    assert(evaluation.interrupted_operation_count == expectedInterrupted);
...
adaptive = search_stage_c_simultaneous_complete_reschedule( ...
    first.next_fault_state.current_plan_view, coreFrozen, adaptiveOptions);
...
timeLimited = search_stage_c_simultaneous_complete_reschedule( ...
    first.next_fault_state.current_plan_view, coreFrozen, timeOptions);
...
fprintf('test_stage_cseq2_restricted_search_contract passed\n');
